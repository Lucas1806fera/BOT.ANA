	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Falo cuzido @${num.split('@')[0]} vai fazer falta nenhuma lisho 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '⚠️ Este comando só pode ser usado pelo meu dono ! 🙈',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ Este comando só pode ser usado pelo ADM, poder nenhum 😹',
					Badmin: '❌ Este comando só pode ser usado quando eu vira ADM na boca do menó! ❌'
				}
			}
			...
			switch(command) {
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Porno uma hora dessas menó anão mané'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Tem que mandar o link junto tiktoker burro')
